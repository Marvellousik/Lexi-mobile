  ...
  const renderContent = () => (
    <View style={styles.tabContent}>
      {state.routes.map((route, index) => {
        ...
        // Bulletproof active state check using both state index and the actual URL path
        const isFocused = state.index === index || pathname.startsWith(`/${route.name}`);
        ...
        return (
          ...
          >
            <View style={[styles.iconPill, isFocused && styles.activePill]}>
              <TabIcon routeName={route.name} color={isFocused ? activeColor : inactiveColor} />
            </View>
